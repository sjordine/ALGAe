 ...
 * @author sergio
 */
public class BestPartialAlignmentAnchorCrossOver implements CrossOverOperator {
    ...
    public List<Chromossome> execute(Chromossome c1, Chromossome c2) {
        ...
        mountAlignment(childData, usedSequences, 0, pivotAlignment, anchorIndex[0], firstParentSequences, subAlignmentAnchorIndex[0]);        
        mountAlignment(childData, usedSequences, 1, pivotAlignment, anchorIndex[1], secondParentSequences, subAlignmentAnchorIndex[1]);
        ...
        child.setOrigin("BPAA_COV");
        child.setRootOrigin("BPAA_COV");
    ...
    private void mountAlignment(ArrayList<ArrayList<Character>> childData, Map<Integer, Integer> usedSequences, int subAlignmentId,
            char[][] pivotAlignment, int anchorIndex, char[][] subAlignment, int subAlignmentAnchor) {
        int pivotCursor = 0;
        int subAlignmentCursor = 0;
        int subAlignmentIndex = 0;

        int numberOfSequences = pivotAlignment.length;
        ...
        while (pivotCursor < pivotAlignment[0].length && subAlignmentCursor < subAlignment[0].length) {
            ...
            if (pivotAlignment[anchorIndex][pivotCursor] == subAlignment[subAlignmentAnchor][subAlignmentCursor]) {
                ...
                if (pivotAlignment[anchorIndex][pivotCursor] != subAlignment[subAlignmentAnchor][subAlignmentCursor] &&
                        pivotAlignment[anchorIndex][pivotCursor]!='-' &&
                        subAlignment[subAlignmentAnchor][subAlignmentCursor]!='-') {
                    System.out.println("ops!!!!!!!!!!!! "+pivotAlignment[anchorIndex][pivotCursor]+","+subAlignment[subAlignmentAnchor][subAlignmentCursor]);
