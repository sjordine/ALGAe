 ...
 * @author sergio
 */
public class LocalAlignmentWithoutGaps extends PairWiseAlignment{
 ...
 private Sequence sequence1;
    ...
    public LocalAlignmentWithoutGaps(Sequence sequence1, Sequence sequence2, Matrix scoreMatrix){
         ...
         if (sequence1!=null && sequence2!=null && scoreMatrix != null &&
            ...
            for (int i=1; i <= seq1.length; i++){
                for (int j=1; j <= seq2.length; j++){
                    matrix[i][j] = Math.max(0,matrix[i-1][j-1] + MatrixUtil.getScore(scoreMatrix, seq1[i-1], seq2[j-1]));
                }
            }
